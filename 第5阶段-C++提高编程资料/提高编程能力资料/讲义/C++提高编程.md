...
// c++的char[]和char*的区别
// https://blog.csdn.net/u013654125/article/details/79758286
// 注意: 不同的C++编译器返回的结果是不同的
```
...
void test01()
{
	...
	//预留空间
    // 比方说, 我们一开始就知道需要多少空间, 就去设置它预留空间是多大
	v.reserve(100000);
...
* `deque<T>` deqT;                      //默认构造形式
* `deque(beg, end);`                  //构造函数将[beg, end)区间中的元素拷贝给本身。区间的方式来赋值
...
// 如果想限定传入的deque只读, 那么用const修饰形参, 并且在使用迭代器的时候, 使用const_iterator来迭代
